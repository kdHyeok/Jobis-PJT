...
공고 수집·OCR·적재·색인을 Airflow DAG 5개로 나눴습니다. 이어 붙이는 방식이 **시각이
아니라 데이터셋 이벤트**입니다.

```
jobis_collect          cron 0 3 * * *          ← 시각 기반은 이 하나뿐
   │  사이트 5곳 병렬 수집
   ▼  Dataset: jobis://raw-site-postings
jobis_load_postgres    Dataset 트리거
   │  canonical 공고로 변환해 job_postings 에 UPSERT
   ▼  Dataset: jobis://job-postings-loaded
jobis_ocr              Dataset + cron */30
   │  이미지 공고를 사이트별로 선점해 텍스트 인식
   ▼  Dataset: jobis://postings-ready-for-rag
jobis_rag              Dataset 트리거
   │  신규·변경 청크만 임베딩해 pgvector 적재
   ▼  Dataset: jobis://rag-index-ready

jobis_legacy_import    schedule=None (수동 전용) — 레거시 SQLite 일회성 이관
```

#### 설계 판단

**1. 후속 DAG을 시각이 아니라 선행 성공에 걸었습니다.**
`schedule=[RAW_POSTINGS]`처럼 데이터셋을 구독합니다. 크롤링 소요 시간은 사이트 응답에
따라 흔들리는데, 후속 DAG을 "3시 30분"으로 고정하면 앞이 늦어진 날 **빈 입력으로 돌아
성공한 것처럼 끝납니다.** 성공 경계로 연결하면 그 조용한 실패가 생기지 않습니다.

**2. 수집은 일부 사이트가 실패해도 다음 단계로 넘깁니다.**
5개 크롤 태스크를 모으는 지점에 `trigger_rule="all_done"`을 두었습니다. 사이트 한 곳의
구조 변경이나 차단 때문에 그날 수집 전체를 버리는 것이 손실이 더 크다는 판단입니다.
실패한 사이트는 그 태스크가 빨간색으로 남아 드러납니다.

**3. 무거운 태스크는 단일 슬롯 풀로 직렬화했습니다.**
OCR과 RAG 적재를 `heavy_memory_pool`(슬롯 1개)에 함께 넣어 동시에 돌지 않게 했습니다.
둘이 겹쳐 호스트 메모리를 소진시킨 장애(2026-08-06)가 근거입니다. RAG 적재는 청크 벡터를
모아 한 번에 커밋하다 7.1GB까지 자랐습니다. 지금은 컨테이너에 `mem_limit`을 걸어
**한계를 넘으면 그 컨테이너만 죽고 태스크가 실패로 남습니다** — 서버는 영향받지
않습니다. 모든 DAG의 `max_active_runs=1`도 같은 이유입니다. 단일 인스턴스에서 파이프라인이
애플리케이션과 자원을 공유하므로, 처리량보다 운영 중 서비스를 지키는 쪽을 택했습니다.

**4. 이벤트 구동이 원칙이지만 `jobis_ocr`에는 예외를 두었습니다.**
`DatasetOrTimeSchedule`로 데이터셋과 30분 주기를 함께 걸었습니다. OCR 대기 행은 수집
경로뿐 아니라 다른 경로로도 생기므로, 데이터셋만 구독하면 그 행이 영구히 방치됩니다.
예외를 두는 대신 코드에 그 이유를 남겼습니다.

**5. 재시도를 두 층으로 나눴습니다.**
Airflow 레벨은 `retries=1`(10분 후)로 일시적 실패만 흡수합니다. OCR은 그것으로 부족해
DB 컬럼에 상태를 두고 **상한 있는 지수 백오프**(기본값 ×2, 최대 24시간)로 재시도하며,
시도 횟수를 넘기면 소진 처리합니다. 행 선점은 `FOR UPDATE SKIP LOCKED`와 lease로
처리해 같은 공고를 두 워커가 잡지 않습니다.

파이프라인은 애플리케이션과 **별도 Compose 묶음**입니다. 색인이 멈춰도 서비스는 계속
동작하며, 검색 실패는 경고로 노출됩니다.
