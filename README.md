...
업로드한 자료를 커리어 조각으로 분해해 저장하며, 중복은 병합하되 **되돌릴 수 있게**
남깁니다.

### 3. 커리어 분석 — 커리어 저장소 기반 역량 분석

![커리어 분석](docs/demo/3-커리어분석.webm)

수집한 원문을 사용자가 확인한 뒤 분석이 시작됩니다. 근거가 부족하면 실패로 처리하지 않고
`답변 필요` 상태로 되묻습니다.

### 4. 공고 분석 및 로드맵 제안 — 커리어 상담과 에이전트 산출물

![대화](docs/demo/4-대화.webm)

사용자 메시지는 즉시 저장되고 AI 응답은 비동기로 도착합니다. AI가 지연되거나 실패해도
메시지는 유실되지 않습니다.
...
로드맵 단계를 학습 플랜으로 펼치고, 채팅형 학습 워크스페이스에서 개념 질문·과제·퀴즈를
진행합니다. 완료 상태는 커리어 지도 노드에 즉시 반영됩니다.
