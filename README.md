...
**시각이 아니라 선행 성공에 연결** — 크롤링 소요 시간은 사이트 응답에 따라 흔들립니다.
후속 DAG을 "3시 30분"으로 고정하면 앞이 늦어진 날 빈 입력으로 돌아 **성공한 것처럼
끝납니다.**

**일부 사이트 실패는 흡수** — 크롤 태스크를 모으는 지점에 `trigger_rule="all_done"`.
한 곳의 차단 때문에 그날 수집 전체를 버리는 손실이 더 큽니다.

**무거운 태스크는 단일 슬롯 풀** — OCR과 RAG 적재를 `heavy_memory_pool`(슬롯 1개)로
직렬화하고 `max_active_runs=1`을 걸었습니다. 둘이 겹쳐 호스트 메모리를 소진시킨
장애(2026-08-06)가 근거입니다. 컨테이너 `mem_limit`을 함께 두어 한계를 넘으면 그
컨테이너만 죽습니다. 단일 인스턴스이므로 처리량보다 운영 중 서비스를 택했습니다.

**`jobis_ocr`만 cron 폴백** — OCR 대기 행은 수집 경로 밖에서도 생기므로 데이터셋만
구독하면 영구 방치됩니다. 이벤트 구동 원칙의 명시적 예외입니다.

**재시도는 두 층** — Airflow `retries=1`은 일시적 실패만 흡수하고, OCR은 DB 상태와
상한 있는 지수 백오프(최대 24시간)로 따로 재시도합니다. 행 선점은
`FOR UPDATE SKIP LOCKED`와 lease로 처리합니다.

파이프라인은 애플리케이션과 별도 Compose 묶음이라, 색인이 멈춰도 서비스는 계속 동작합니다.
