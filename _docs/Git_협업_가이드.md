...
| 브랜치 | 역할 | 규칙 |
|---|---|---|
| `master` | 배포·시연 가능한 안정 버전 | 직접 Push 금지. 통합 테스트 끝난 `develop`만 MR로 병합. 배포 시 태그 생성 (`v0.1.0`) |
| `develop` | 통합 개발 기준 브랜치 | 직접 Push 금지. 모든 작업 브랜치의 시작점. 실행 안 되는 코드 병합 금지 |
| `feat/<영역>/<기능명>` | 새 기능 개발 | 영역: `ai` `be` `fe` `infra`. 최신 `develop`에서 분기 |
...
나쁜 예: donghyeok, backend, final, develop2, jobis-final-final
```
...
- 최소 1명의 리뷰 후 병합 (본인 MR 셀프 승인·병합 금지)
- 병합 옵션: **Squash commits** + **Delete source branch** 체크
...
| **Bugfix** | 아직 배포되지 않은 개발 버전의 오류 수정 | `develop` |
...
**필수는 두 섹션뿐입니다.** 나머지는 해당 없으면 섹션째 삭제하세요.

| 섹션 | 필수 | 작성 요령 |
|---|---|---|
| 작업 내용 | ✅ | 핵심 변경만 1~3개 |
| 확인 방법 | ✅ | 리뷰어가 그대로 따라 할 최소 단계 |
| 관련 이슈 | 선택 | `Jira: JOBIS-123` 번호만. 이슈 없으면 삭제 |
| 변경 영향 | 선택 | API/DB/환경변수/화면/의존성 중 해당 항목만 체크. 영향 없으면 삭제 |
| 참고 사항 | 선택 | 제한 사항·후속 작업. 없으면 삭제 |
| 체크리스트 | ✅ | 실제로 확인한 것만 체크 (허위 체크 금지) |
...
1. `master`·`develop`에 직접 Push하지 않는다.
2. 모든 작업 브랜치는 최신 `develop`에서 생성한다. (hotfix만 `master`)
3. MR은 최소 1명 리뷰 후 Squash 병합하고, Source 브랜치는 삭제한다.
4. 병합 전 작성자가 직접 실행·테스트한다. 실행 안 되는 코드는 병합하지 않는다.
5. 공통 규격(API·DB·환경 변수) 변경은 담당자에게 공유하고 MR에 명시한다.
6. Hotfix는 `master`와 `develop` 양쪽에 반영한다.
