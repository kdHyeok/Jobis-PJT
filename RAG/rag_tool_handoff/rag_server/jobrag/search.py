...
RERANK_TOP_N = 30   # 재순위 대상 상한 = 후보 전량. 근거는 아래.
#
# 2026-07-30 실측(`eval/knob_ab.json`, 60질의, 판정 재사용)으로 15 -> 30 상향.
#
#   K/N     원nDCG@3   정규@3   nDCG@10        P@3
#   30/15    0.7670    0.7090   0.7112      0.6889   (이전)
#   30/30    0.7869    0.7372   0.7427*     0.7056   <- 채택
#   50/30    0.7649    0.7093   0.7360      0.6778
#   50/50    0.7632    0.7087   0.7308      0.6778
#   (* k=10에서 Δ+0.0313, Bonferroni 보정 후에도 유의. k=3은 +0.0197 미유의)
#
# 왜 N만 올리고 CANDIDATE_K는 30으로 두는가:
# 적합 공고의 RRF 융합 순위가 p50 15위라 절단선이 중위수에 앉아 있었다 — 후보에 들어온
# 적합의 49.9%가 CE를 보지 못했다(`eval/rerank_depth_curve.json`). N=30이면 도달률이
# 0.5323 -> 0.8165로 오른다. 반면 K를 50으로 늘리면 후보가 늘어난 만큼 적합의 융합 순위가
# 뒤로 밀려(p50 15->18위) 도달률이 오히려 떨어지고(0.5022), 실측도 개선이 없었다.
# 즉 병목은 후보 수가 아니라 **재순위 절단**이었다.
#
# 비용: 재순위 지연 1,310 -> 1,730ms (+420ms). CE 호출이 15 -> 30건으로 두 배다.
# 되돌리기: `revert_config.py --to 20260730_before_knob_expansion`
