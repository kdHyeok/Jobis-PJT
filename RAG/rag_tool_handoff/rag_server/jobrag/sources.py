...
# ── 본문 보강 추출 ─────────────────────────────────────
# 크롤러가 experience 필드에 값 없이 라벨만 담는 경우가 매우 많다("경력" 단독 785건/67%).
# 그러면 parse_exp_range가 None을 반환하고 search.py의 `exp_min IS NULL OR ...` 조건이
# 무조건 통과시켜 연차 필터가 코퍼스 79%에서 무력화된다. 요구 연차는 본문에 적혀 있으므로
# (NULL 930건 중 476건) 본문에서 보강 추출한다.
#
# 오추출 위험이 실재하므로 보수적으로 간다:
#   - "경력"과 숫자가 인접한 표현만 채택 (복리후생의 "5년마다 장기근속" 등 배제)
#   - 뒤에 우대/선호가 붙으면 요구가 아니라 가점이므로 제외
#   - 후보가 여러 개면 최솟값 채택 (과도한 필터링으로 적합 공고를 떨어뜨리지 않기 위해)
_BODY_EXP_RES = (
    re.compile(r"경력\s*\(?\s*(?:최소\s*)?(\d{1,2})\s*년\s*(?:이상|이상부터|↑|\+)"),
    re.compile(r"(\d{1,2})\s*년\s*(?:이상|↑)\s*(?:의\s*)?(?:경력|실무|근무\s*경험)"),
    re.compile(r"경력\s*(\d{1,2})\s*[-~]\s*\d{1,2}\s*년"),
    re.compile(r"경력\s*(\d{1,2})\s*년"),
)
_PREFER_WORDS = ("우대", "선호", "플러스", "가산", "있으면 좋")


def parse_exp_from_body(text: str) -> int | None:
    """본문에서 요구 최소 연차를 보강 추출. 못 찾으면 None.

    experience 필드가 숫자를 잃은 공고를 구제하는 용도다. 확실한 표현만 취하고
    애매하면 None을 반환한다 — 틀린 값을 넣는 것이 NULL보다 나쁘다.
    """
    if not text:
        return None
    found = []
    for rx in _BODY_EXP_RES:
        for m in rx.finditer(text):
            tail = text[m.end():m.end() + 18]
            if any(w in tail for w in _PREFER_WORDS):
                continue          # "경력 5년 이상 우대" = 요구가 아니라 가점
            n = int(m.group(1))
            if 0 < n <= 20:       # 21년+ 는 오추출로 본다
                found.append(n)
...
def to_posting(rec: dict) -> Posting:
    raw_exp = rec.get("experience") or ""
    exp_min, exp_max, exp_suspect = parse_exp_range(raw_exp)
    regions = parse_regions(rec.get("location") or "")
    title = rec.get("title", "")
    detail_text = rec.get("detail_text") or ""

    # experience 필드가 숫자를 잃었을 때만 본문으로 보강한다.
    # "경력무관"/"신입"은 파서가 의도적으로 None/0을 준 것이므로 덮지 않는다.
    if exp_min is None and "무관" not in raw_exp and "신입" not in raw_exp:
        body_min = parse_exp_from_body(f"{title}\n{detail_text}")
        if body_min is not None:
            exp_min = body_min
            exp_suspect = True     # 본문 추정치임을 표시 — 감사 가능하게 남긴다
