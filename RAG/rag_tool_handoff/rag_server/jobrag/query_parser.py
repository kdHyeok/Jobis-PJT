...
class QuerySpec:
    text: str
    # 의미 축(dense 임베딩 + 크로스인코더) 전용 질의. None이면 text로 폴백한다.
    #
    # 왜 나누는가: text 하나가 BM25·dense·재순위 세 곳에 동시에 쓰이는데, 어휘 축과
    # 의미 축이 원하는 질의가 다르다. BM25는 토큰이 많을수록 재현율이 오르지만,
    # 임베딩은 범용 토큰(Python·Git·Linux)이 붙으면 벡터가 "일반 IT 공고" 쪽으로
    # 끌려가 직군 판별력을 잃는다. 실측: 입력 B에서 dense가 입력 A보다 −0.303
    # (95% CI [−0.492, −0.133]) 낮은데 bm25는 −0.043으로 거의 무변화였다.
    # 상세: eval/PREREG_query_text.md
    ...
    def semantic_text(self) -> str:
        """의미 축이 실제로 쓰는 질의. dense_text가 비어 있으면 text와 동일."""
        return (self.dense_text or self.text)
