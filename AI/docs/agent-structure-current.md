...
**측정(2026-07-24, `evals/planner_dataset.json` 41케이스)**: 플래너 경로 시퀀스 정확도
**41/41 (100%)**, 되묻기 일치율 100%. baseline: `evals/planner_baseline.json`.
...
| 오라우팅 | 라우팅 가시화 + 저신뢰 되묻기 + 결정론 검증기(validate_plan) + 플래너 평가셋(41케이스 100%) |
...
| 플래너 | 발화→기대 실행 시퀀스 평가셋 41케이스 (`eval/planner_harness.py`, 실 LLM) | ✅ 정확도 100% (07-24) |
