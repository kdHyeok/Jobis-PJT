...
- 순서: (진단) → 무엇이 확인됐는지 → 무엇이 부족한지 → 계획이 어떻게 되는지.
...
class _Summary(BaseModel):
    ...
    summary: str = Field(default="", description=(
        "사용자에게 보여줄 판정 요약 본문. 확인된 것 → 부족한 것 → 준비 계획 순서로 "
        "여러 문장으로 쓴다. 제목이나 한 줄 요약이 아니다. 입력에 없는 사실·수치를 넣지 않는다."))
...
def _deterministic_summary(facts: dict) -> str:
    ...
    gaps = facts.get("gapCount", 0)
    undecided = facts.get("undecidedCount", 0)
    items = facts.get("roadmapItemCount", 0)
    weeks = facts.get("totalWeeks", 0)
    ...
    if items:
        parts.append(f"이를 보완하기 위해 {weeks}주 동안 {items}개 항목의 학습 계획을 구성했습니다.")
    else:
        parts.append("현재 정보로는 별도 학습 계획을 구성하지 않았습니다.")
    return " ".join(parts)


def _collect_facts(gap: dict, roadmap: dict, posting: dict) -> dict:
    ...
    return {
        ...
        "gapCount": len(gap.get("gaps", [])),
        "strengths": [s.get("text", "") for s in gap.get("strengths", [])],
        "gapReasons": [g.get("reason", "") for g in gap.get("gaps", [])],
        "roadmapItemCount": len(roadmap.get("roadmap", [])),
        "roadmapTitles": [i.get("title", "") for i in roadmap.get("roadmap", [])],
        "totalWeeks": roadmap.get("totalWeeks", 0),
    }


def render_summary(gap: dict, roadmap: dict, posting: dict) -> tuple[str, list[dict]]:
    """확정된 분석 결과 → 사람이 읽는 요약 한 문단.

    반환: (요약, warnings). 어떤 경우에도 빈 문자열을 돌려주지 않는다.
    """

    facts = _collect_facts(gap, roadmap, posting)
