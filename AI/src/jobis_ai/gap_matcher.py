...
from jobis_ai.semantic_judge import judge_topics_relevance
...
class GapMatcher:
    ...
    def _semantic_outcome(
        related_idx: list[int] | None, evidences: list[dict]
    ) -> tuple[str, list[str], float, str]:
        """LLM 의미 판정 결과(관련 문장 인덱스) → 매칭 상태.
        ...
        LLM은 이진 판정만 한다(met/not_met) — "얼마나 충족하는지" 등급을 안 매기고,
        None(판정 불가)은 not_met 이 아니라 uncertain 이다(모른다/아니다 구분).
        """
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _domain_keyword_rule_ids(keyword: str, evidences: list[dict]) -> list[str]:
        """도메인 키워드 1차 포함 매칭 (룰, 무료·즉시·결정론). 실패 시 빈 목록 —
        그 건은 LLM 의미 판정 배치로 넘어간다(match 참고)."""
    ...
    def match(self, requirements: list[dict], profile: dict) -> MatchReport:
        ...
        llm_unavailable = 0
        # LLM 의미 판정이 필요한 건들을 모아 뒀다가 **한 번의 배치 호출**로 처리한다
        # (2026-07-23 성능 개선 — topic 마다 개별 호출하면 요구사항 많은 공고에서 수 분 소요).
        # (report.matches 인덱스, topic 문장, 종류) — 판정 기준은 단건 시절과 동일하다.
        pending_semantic: list[tuple[int, str, str]] = []
        ...
        for req in requirements:
            ...
            if kind == "domain_keyword":
                matched_ids = self._domain_keyword_rule_ids(text, evidences)
                if matched_ids:
                    report.matches.append(Match(
                        requirementId=req_id, type=req_type, text=text, status="met",
                        kind="domain_keyword",
                        matchedEvidenceIds=matched_ids, confidence=1.0, method="keyword",
                        reason=_reason_for_domain_keyword(text, "met", "keyword"),
                    ))
                else:
                    # 포함 매칭(룰) 실패 → LLM 의미 판정 배치로. 우선 uncertain 자리를 잡는다.
                    report.matches.append(Match(
                        requirementId=req_id, type=req_type, text=text, status="uncertain",
                        kind="domain_keyword", confidence=0.0, method="undecidable",
                        reason=_reason_for_domain_keyword(text, "uncertain", "undecidable"),
                    ))
                    pending_semantic.append((len(report.matches) - 1, text, "domain_keyword"))
                continue
            ...
            # 기술명이 안 뽑히는 서술형 요구사항 → 2차 LLM 의미 판정 배치로.
            report.matches.append(Match(
                requirementId=req_id, type=req_type, text=text, status="uncertain",
                confidence=0.0, method="undecidable",
                reason=_reason_for_llm_semantic("uncertain", "undecidable"),
            ))
            pending_semantic.append((len(report.matches) - 1, text, "text"))
        ...
        if pending_semantic:
            ...
            for pos, (match_idx, topic, topic_kind) in enumerate(pending_semantic):
                ...
                if sem_method == "undecidable":
                    llm_unavailable += 1
                    continue  # 자리 잡아 둔 uncertain 그대로
                report.matches[match_idx] = replace(
                    report.matches[match_idx],
                    status=status, matchedEvidenceIds=matched_ids,
                    confidence=confidence, method=sem_method,
                    reason=(
                        _reason_for_domain_keyword(topic, status, sem_method)
                        if topic_kind == "domain_keyword"
                        else _reason_for_llm_semantic(status, sem_method)
                    ),
                )
        ...
        trace.emit("judgment", f"매칭 캐스케이드 판정 {len(report.matches)}건", {
            "matches": [
                {
                    "requirementId": m.requirementId, "text": m.text, "type": m.type,
                    "kind": m.kind, "status": m.status, "method": m.method,
                    "confidence": m.confidence, "matchedSkills": m.matchedSkills,
                    "missingSkills": m.missingSkills,
                    "matchedEvidenceIds": m.matchedEvidenceIds, "reason": m.reason,
                }
...
def overall_fit(score_basis: dict) -> tuple[float | None, str]:
    ...
    **계산된 카테고리만** 골라 가중 평균한다(None 은 '계산 근거 없음'이므로 분모에서 뺀다 —
    ...
    if not present:
        trace.emit("score", "계산 가능한 카테고리 없음 → 판정불가", {
            "categoryScores": dict(score_basis), "grade": "판정불가",
        })
        return None, "판정불가"
    ...
    if score >= GRADE_HIGH:
        ...
        grade = "하"
    trace.emit("score", f"종합 점수 {score} → 등급 {grade}", {
        "categoryScores": dict(score_basis),
