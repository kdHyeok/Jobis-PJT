...
def get_agent_registry() -> dict[str, AgentSpec]:
    ...
    specs = [
        ...
        AgentSpec(
            ...
            description=("공고만으로 핵심 요구사항 정리(요구 연차·기술 스택·필수/우대) — 이력서 없이 가능, "
                         "판정은 안 함. **이력서가 없어 적합도 판정을 못 할 때 공고로 먼저 할 수 있는 일**이다"),
            preconditions=("job_posting",),
            entry=posting_analysis.run,
            kind="tool",
            render=tool_render.render_posting_analysis,
        ),
        AgentSpec(
            name="job_recommend",
        ...
        AgentSpec(
            ...
            description=("이력서 진단 — 강점·빈약 항목·보완 질문. **공고가 없어 적합도 판정·자소서·면접을 "
                         "못 할 때 이력서로 먼저 할 수 있는 일**이다"),
            preconditions=("resume",),
            entry=resume_diagnosis.run,
            kind="tool",
            render=tool_render.render_resume_diagnosis,
        ),
        AgentSpec(
            name="interview_prep",
