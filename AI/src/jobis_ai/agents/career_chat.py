...
class _ChatWrite(BaseModel):
    """표현 전용 — 대화 응답. 판단 필드 없음.

    **description 을 반드시 유지한다.** 구조화 출력에서 필드 설명이 없으면 모델이 이 칸을
    '제목'처럼 취급해 한 줄만 채운다(실측: 설명 없음 19~43자 → 설명 있음 1400자대).
    """

    reply: str = Field(default="", description=(
        "사용자에게 그대로 보여줄 답변 본문. 고민·하소연이면 공감 두세 문장. "
        "정보성 질문이면 번호 목록 3~6개로 각 항목에 무엇을 어떻게 할지 구체적으로 쓴다. "
        "인사말이나 '안내해드릴게요' 같은 약속만 있는 짧은 답은 금지."))


_SYSTEM = """너는 취업 준비생과 진로 이야기를 나누는 상담가다.
