...
### 통합 실험판 LLM (`codex_cli` 고정)

`scripts/start-ai-agent.ps1`은 오래된 `.env`에 `claude_code`가 남아 있어도 기존 AI 서버의
LLM 공급자를 `codex_cli`로 덮어쓴다. 따라서 Claude CLI 로그인 상태와 무관하게 동작한다.
Codex CLI는 긴 공고를 명령줄 인자가 아닌 표준입력으로 받고, 구조화 호출에는 JSON Schema를
사용한다.
...
CODEX_MODEL_LIGHT=gpt-5.6-luna
CODEX_MODEL_ROUTER=gpt-5.6-luna
CODEX_EFFORT=low
```

설정을 바꾼 뒤에는 브릿지를 **재시작**해야 적용된다.
`GMS_KEY` 가 없어도 임베딩 계층은 예외를 던지지 않고 경고만 남기고 건너뛴다
(`embed_impl/gms_openai.py` — "예외를 던지지 않는다" 규칙). 즉 GMS 키 없이도 Codex 기반
대화·구조화 분석은 동작한다.
