...
위 `false`는 별도 저장소를 제공할 수 없는 제한된 서버에서 명시적으로 선택하는 저하 모드다.
논리 오류, 잘못된 마이그레이션과 운영자 실수는 dump와 격리 복원으로 대응할 수 있지만 호스트
디스크 장애에는 대응하지 못한다. 감사와 준비 스크립트는 이 경우 `[WARN]`을 남기며, 기본 예제의
`true`와 별도 파일시스템 강제 동작은 유지된다. develop에서 만든 SHA 이미지가 서버 Docker
daemon에 생긴 뒤, 다음 명령이 레거시 프로세스를 멈추지 않은 채 Flyway V27 적용,
