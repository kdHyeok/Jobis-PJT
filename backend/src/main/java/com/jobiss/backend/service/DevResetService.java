...
public class DevResetService {
    ...
    public Map<String, Object> resetByEmail(String email) {
        ...
        // 대화(V7) — 분석런이 먼저 지워져야 FK(analysis_runs.conversation_id)가 풀린다
        exec("delete from conversation_messages where conversation_id in "
                + "(select id from conversations where user_id = " + uid + ")");
        int conversations = exec("delete from conversations where user_id = " + uid);
        int evidences = exec("delete from evidences where user_id = " + uid);
        int resumes = exec("delete from resume_documents where user_id = " + uid);  // 이력서 원문(V6)도 함께
        ...
        out.put("resumeDocuments", resumes);
