...
public class DataInitializer implements CommandLineRunner {
    ...
    private void seedDemoUser() {
        ...
        evidenceRepository.save(evidence(user, EvidenceKind.PROJECT, "Spring Boot 게시판 REST API",
                "Java · Spring Boot · JPA · MySQL — 목록 API 1.2s → 180ms", EvidenceStatus.OK));
        evidenceRepository.save(evidence(user, EvidenceKind.GITHUB, "GitHub — github.com/junyoung-p",
                "커밋 214건 · 테스트 폴더 없음 · 단독 커밋 96%", EvidenceStatus.OK));
        evidenceRepository.save(evidence(user, EvidenceKind.PORTFOLIO, "포트폴리오_박준영_v2.pdf",
                "18쪽 · 성과 수치 포함", EvidenceStatus.OK));
        evidenceRepository.save(evidence(user, EvidenceKind.STACK, "기술 스택",
                "Java · Spring · MySQL · Python", EvidenceStatus.OK));
        evidenceRepository.save(evidence(user, EvidenceKind.EDU, "교육 이력",
                "SSAFY / KDT 지원 준비 중", EvidenceStatus.PARTIAL));
        evidenceRepository.save(evidence(user, EvidenceKind.CERT, "자격증 · 어학",
                "미입력 — 추가 질문 필요", EvidenceStatus.MISSING));
    }

    private Evidence evidence(User user, EvidenceKind kind, String label, String desc, EvidenceStatus status) {
        return Evidence.builder()
                .user(user).kind(kind).label(label).description(desc).status(status).build();
    }
}
