...
- 현재 상태: Phase 0~7 AI 코어 완료, Phase 8 채용공고·분석·로드맵 통합판 구현 및 검증 중
...
- [x] 채팅 공고 첨부와 채용공고 페이지가 같은 추출 API를 사용
- [ ] URL 입력 후 추출 원문을 보고 수정·확인 가능
- [ ] 이미지 원본과 경력·직무 근거 영역을 함께 확인 가능
- [x] 새로고침 후에도 확인 대기 상태 복원
...
- [x] 별도 `C:\jobiss-capability-graph-lab`의 계약과 버전 확인
...
2026-08-04 원자 역량 그래프 `0.1.0-alpha.1`과 실제 HTTP 호출을 검증했다. v3 수신 계약은
고정 level을 제거하고 원자 역량의 목표·검증 방법·기술 컨테이너, 프로젝트 과제 target,
위상 정렬 학습 순서와 전체 응답 hash를 소비한다. 다음 게이트는 기존 Spring catalog의 기술
묶음을 원자 역량과 사용자별 프로젝트 초안으로 전환하는 것이다.

### 구현

- [x] 학습 가능한 requirement만 curriculum 입력으로 선택
- [x] 원자 역량별 scopeDefinition 사용 및 고정 level 제거
- [x] 기존 canonical node 재사용 후보
- [x] 선행관계와 보너스 확장 후보
- [x] 포지션별 회사 맞춤 프로젝트 제안
- [x] 경력 관문 입력 생성
- [x] excluded requirement와 이유 저장
- [x] 백엔드 그래프 compiler 연결
- [x] 미리보기·적용·취소 상태 연결
...
- [x] Capability Graph의 승인된 원자 역량 catalog 직접 조회
- [x] 넓은 공고 요구사항을 회사 맞춤 프로젝트 과제와 원자 역량으로 분해
- [x] 과제에서 선택한 원자 역량만 선행 graph closure 대상으로 사용
- [x] 공용 graph에 없는 역량은 임의 연결하지 않고 검토 대상으로 격리
- [x] Spring 분석 요청에서 중복 broad capability catalog 제거

이 참조 compiler는 Spring이 포팅할 의미 기준이며 AI API가 공개 지도를 적용한다는 뜻이 아니다.
`C:\jobiss-service-v3-integration-lab`의 Spring·DB·Vue에 별도 통합했으며 두 원본 작업공간은
변경하지 않았다.

### 구현

- [x] `AI_PROVIDER=legacy|v3|shadow` 지원
- [x] 채팅의 명시적 공고 첨부 진입점 연결
- [x] 채용공고 페이지 진입점 연결
- [x] 원문 확인 카드 UI
- [x] 포지션 선택 카드 UI
- [x] 실제 단계 기반 진행 시각화
- [x] 상세 실행 과정 접기/펼치기
- [x] 프로젝트 노드 상세에서 과제·완료 기준·원자 역량 표시
- [x] 로드맵 operation·snapshot에 graph 노드 버전·기술 컨테이너·검증 방법 보존
- [x] 사용자별 원자 역량 상태·감사 event·legacy 이관 후보 RLS 저장소
- [x] 로드맵 적용 시 원자 상태 등록, 공개 지도 조회와 다음 분석 입력에 최신 상태 반영
- [x] 이관 후보의 사용자 확인과 원자 역량 상태 전이 API
- [x] 원자 역량별 범위 제한 문제·코드 검증 및 이의제기
- [x] 상태 조회와 새로고침 복원
- [x] 원자 역량 검증 중단·재시도·새로고침 복원
- [x] 분석 결과와 로드맵 초안 명칭 분리
- [x] 운영자 원자 역량 검증 이의제기 검토 화면
- [x] 운영자 신규 기술·이상 추출 검토 화면

### 자동 검증

- [x] 채팅과 공고 페이지가 같은 공고에 같은 구조화 결과를 사용
- [x] 페이지 이동과 새로고침 후 작업 상태 일치
- [x] 취소된 작업의 늦은 응답이 성공으로 덮어쓰지 못함
- [x] stale RUNNING 작업 lease 복구
- [x] 동일 action 재전송이 중복 작업을 만들지 않음
- [x] CORS·인증·RLS 기본 회귀 없음
...
원자 역량 검증 하위 흐름은 2026-08-04 격리 포트에서 실제 HTTP·PostgreSQL E2E를
통과했다. 자기 확인, 2문항 통과와 재학습, 재로그인 복원, 사용자 RLS, 운영자 이의 승인,
legacy 후보의 `EVIDENCED` 전이까지 확인했다. 전체 공고 진입점 E2E와 provider 비교·전환은
Phase 9에서 계속한다.

## Phase 9. 비교 평가와 전환

- [x] 공고 원문 재전송 없는 passive paired 비교 저장소와 운영 흐름
- [x] 공개 합성 LEGACY/V3 결과 쌍 5건의 결정론적 field diff 회귀
- [x] 공개 합성 공고 1건의 실제 LEGACY/V3 완료와 passive paired 자동 비교
- [ ] 같은 공개 공고 fixture를 실제 legacy와 v3 모델에 반복 실행
- [x] 필드별 일치도·역량 집합 유사도·계약 비교
- [ ] 정답표와 사람 판정에 의한 정확성·근거 완전성·안정성 비교
- [ ] 기존이 맞고 v3가 틀린 사례 분석
- [ ] 기존이 틀리고 v3가 고친 사례 기록
- [ ] 실제 모델 비용과 p50/p95 시간 측정
- [x] shadow mode 개인정보 중복 전송 금지와 저장 경계 확정
- [x] 운영자 비교·상세·근거 판정 UI와 일반 사용자 403 검증
- [ ] 실제 paired 표본에 대한 운영자·개발자 검토
- [ ] 제한된 내부 사용자에 v3 활성화
- [ ] 오류율·질문율·수정률 관찰
- [ ] 기준 통과 후 기본 provider 전환
- [ ] legacy 제거 여부는 별도 결정

실제 1회 비교는 통합 경로와 실패 복구 결함을 찾는 개발 검증으로만 인정한다. 표본이 한 건이고
측정 기준도 달라 정확도·안정성·p50/p95 판단에는 사용하지 않는다. 결과와 전환 보류 사유는
`docs/16-phase9-live-provider-comparison.md`와 D036에 기록했다.
