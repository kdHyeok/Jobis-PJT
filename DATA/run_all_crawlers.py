...
CRAWLERS = (
    ...
    ("incruit", "crawl_incruit_it.py", ()),
    ("work24", "crawl_work24_it.py", ()),
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="사이트별 검색어마다 조회할 검색 결과 페이지 수 (기본값: 1)",
    )
    parser.add_argument(
        "--max-new",
        type=int,
        default=None,
        help="사이트별로 이번 실행에서 새로 추가할 공고 수 상한(일일 증분 수집용)",
    )
...
def main() -> int:
    ...
    if (
        ...
        raise SystemExit("--max-results와 --pages-per-keyword는 1 이상, --delay는 0.5 이상이어야 합니다.")
    ...
    for source, script_name, extra_args in CRAWLERS:
        command = [
            ...
            "--pages-per-keyword",
            str(args.pages_per_keyword),
            *(["--max-new", str(args.max_new)] if args.max_new is not None else []),
            *extra_args,
        ]
    ...
    if failures:
        # 일부 사이트가 실패해도, 성공한 사이트만으로 병합은 계속 진행한다.
        print(f"[run_all] status=crawlers_completed_with_failures sources={','.join(failures)}", flush=True)
    ...
    # 크롤러 실패가 있었으면 병합은 됐더라도 비정상 종료로 알린다.
    if failures:
