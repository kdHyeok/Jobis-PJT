"""Hermes와 무관한 Codex OAuth + OpenAI 형식 변환 클라이언트."""
...
DEFAULT_SCHEMA_NAME = "ollama_response"
...
def _load() -> dict[str, str] | None:
    ...
    try:
        ...
        raise CodexError("인증 파일이 손상되었습니다. 다시 로그인하세요.") from None
...
def login() -> None:
    ...
    if not authorization:
        raise CodexError("로그인 시간이 초과되었습니다.")
...
def _refresh(tokens: dict[str, str]) -> str:
    ...
    if response.status_code != 200:
        raise CodexError("token을 갱신할 수 없습니다. 다시 로그인하세요.")
    data = response.json()
...
def access_token(force_refresh: bool = False) -> str:
    ...
    if not tokens:
        if not sys.stdin.isatty():
            raise CodexError("인증 정보가 없습니다. `codex-oauth --login`을 실행하세요.")
        login()
        tokens = _load()
    assert tokens
...
def _headers(token: str, accept: str = "text/event-stream") -> dict[str, str]:
    headers = {
        ...
        "Accept": accept,
        "User-Agent": "codex_cli_rs/0.0.0 (standalone codex-oauth-test)",
        "originator": "codex_cli_rs",
    }
    account = _claims(token).get("https://api.openai.com/auth", {}).get("chatgpt_account_id")
    if account:
...
def _tool_choice(choice: Any) -> Any:
    ...
    return {"type": "function", "name": function.get("name")} if function.get("name") else "auto"
...
def _json_schema_format(
    ...
    *,
    ...
    if not isinstance(name, str) or not _SCHEMA_NAME_PATTERN.fullmatch(name):
        raise CodexError("response_format schema name은 영문자, 숫자, '_' 또는 '-'로 된 1~64자여야 합니다.")
...
def build_payload(
    ...
    if not items:
        raise CodexError("messages에 처리할 대화가 없습니다.")
    payload: dict[str, Any] = {
        "model": model,
        "input": items,
        "store": False,
        "stream": True,
        "reasoning": {"effort": "low" if effort == "minimal" else effort, "summary": "auto"},
    }
    if instructions:
    ...
    if converted := _tools(tools):
        ...
        payload["parallel_tool_calls"] = True if parallel_tool_calls is None else parallel_tool_calls
...
def stream_events(
    ...
    payload = build_payload(messages, model, effort, tools, tool_choice, parallel_tool_calls, response_format)
    for attempt in range(2):
        token = access_token(force_refresh=attempt == 1)
        try:
            with httpx.Client(timeout=timeout or 60) as client:
                with client.stream("POST", f"{BASE_URL}/responses", headers=_headers(token), json=payload) as response:
                    if response.status_code == 401 and attempt == 0:
                        continue
                    if response.status_code == 403:
                        raise CodexError("Codex 접근이 거부되었습니다. 계정 권한을 확인하세요.")
                    if response.status_code == 429:
                        raise CodexError("Codex 요청 한도를 초과했습니다.")
                    if response.status_code != 200:
                        raise CodexError(f"Codex 요청 실패: HTTP {response.status_code}")
                    yield from _sse(response.iter_lines())
                    return
        except httpx.HTTPError:
            raise CodexError("Codex 서버에 연결하지 못했습니다.") from None
    raise CodexError("Codex 인증에 실패했습니다. 다시 로그인하세요.")
...
def ask(prompt: str, model: str = DEFAULT_MODEL, effort: str = "medium", timeout: float | None = None) -> str:
    if not prompt.strip():
        raise CodexError("질문이 비어 있습니다.")
    result = complete([{"role": "user", "content": prompt}], model, effort, timeout)
