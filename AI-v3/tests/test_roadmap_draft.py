...
from jobis_ai_v3.contracts.posting import PostingStatus, RequirementCategory, StructuredPosting
...
from jobis_ai_v3.roadmap import RoadmapDraftFailure, RoadmapDraftService
...
def test_roadmap_proposal_endpoint_returns_draft_only(structured_posting) -> None:
    secret = "test-ai-secret-123"
    app = create_app(
        Settings(environment="test", shared_secret=secret, host="127.0.0.1", port=8300),
        roadmap_service=service(),
    )
    body = request(structured_posting).model_dump(mode="json", by_alias=True)
    ...
    assert any(
        item["action"] == "CREATE_TARGET_PROJECT"
